    ...
    class Builder
    {
        ...
        private void SetPrimaryKeys()
        {
            using (var dal = new DAL(connectionString))
            {
                ...
                {
                    var column = table.Columns.Single(c => c.Name.EqualsIgnoreCase(dal.GetPrimaryColumnName(table)));
        ...
        private List<AssociationModel> BuildAssociations(DAL dal)
        {
            ...
            foreach (var rawAssociation in dal.GetRawAssociations())
            {
                ...
                association.PrimaryColumn = association.PrimaryTable.PrimaryKey;
                association.ForeignColumn = association.ForeignTable.Columns.Single(c => c.Name.EqualsIgnoreCase(dal.GetForeignColumnName(association.PrimaryTable, association.ForeignTable)));
